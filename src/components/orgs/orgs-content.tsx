  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      <div className="shrink-0 mb-4">
        <h1 className="text-xl font-medium tracking-tight">Organizations</h1>
        <p className="text-sm text-muted-foreground/70 mt-0.5">
          Browse organizations first, then open each one for repository details
        </p>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto border border-border divide-y divide-border">
        {filtered.map((org) => (
          ...
          >
            <Link href={`/orgs/${org.login}`} className="flex items-center gap-4 flex-1 min-w-0">
              <Image
                src={org.avatar_url}
                alt={org.login}
                width={26}
