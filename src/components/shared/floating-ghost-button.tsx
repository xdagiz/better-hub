  ...
  return (
    ...
    >
      <div className="relative w-4 h-4">
        {/* Ghost icon (stroke outline) */}
        <Ghost
          className={cn(
            "w-4 h-4 absolute inset-0 transition-opacity duration-300",
            state.isWorking ? "opacity-30" : "opacity-100"
          )}
          strokeWidth={2}
        />

        {/* Idle shimmer — subtle light streak that sweeps across once in a while */}
        {!state.isWorking && (
          <svg
        ...
        {/* Working: fill sweep clipped to ghost shape */}
        {state.isWorking && (
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="16"
