            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <DialogTitle className="text-sm font-medium">
                  {step === "templates" ? "New issue" : "Create issue"}
                </DialogTitle>
                ...
                <div className="flex items-center gap-0 mb-1.5 shrink-0">
                  ...
                  {bodyTab === "write" && (
                    <div className="flex items-center gap-0 border-l border-border/30 dark:border-white/5 pl-2">
                      {toolbarActions.map(({ icon: Icon, action, title: t }) => (
                        <button
                          key={t}
                          onClick={action}
                ...
                {!showLabelPicker ? (
                  <div className="flex items-center gap-1.5 min-h-[28px]">
                    ...
                    {selectedLabels.length > 0 ? (
                      <div className="flex items-center gap-1 flex-wrap flex-1 min-w-0">
                        {selectedLabels.map((name) => {
                          const l = repoLabels.find((rl) => rl.name === name);
                          return (
                            <span
                              key={name}
                            ...
                            >
                              <span
                                ...
                                style={l ? { backgroundColor: `#${l.color}` } : undefined}
                              />
                              {name}
                              <button
                    ...
                    {repoLabels.length > 0 && (
                      ...
                      >
                        {selectedLabels.length > 0 ? "Edit" : "Add"}
                      </button>
                    )}
                  </div>
                ) : (
              ...
              <div className="px-4 py-2.5 border-t border-border/40 dark:border-white/5 shrink-0">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-[10px] text-muted-foreground/25">
                    {typeof navigator !== "undefined" && /Mac|iPhone|iPad/.test(navigator.userAgent) ? "⌘" : "Ctrl"}+Enter to submit
                  </span>
