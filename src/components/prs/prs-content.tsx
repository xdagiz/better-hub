...
export function PRsContent({
  ...
  const tabItems: { key: TabType; label: string; icon: React.ReactNode; count: number }[] = [
    { key: "review", label: "Review requested", icon: <Eye className="w-3 h-3" />, count: reviewRequested.total_count },
    ...
    if (search) {
      ...
      list = list.filter(
        (pr) =>
          ...
          extractRepoName(pr.repository_url).toLowerCase().includes(q)
      );
    }
                  ...
                  <div className="flex items-center gap-3 mt-0.5">
                    ...
                    {pr.user && pr.user.login !== username && (
                      <span className="flex items-center gap-1.5 text-[11px] text-muted-foreground/50">
                        ...
                        />
                        <span className="font-mono text-[10px]">{pr.user.login}</span>
                      </span>
                    )}
